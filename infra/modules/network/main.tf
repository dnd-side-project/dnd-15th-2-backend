...
resource "aws_vpc" "this" {
  ...
  # 보완 통제: SSM Session Manager 세션은 CloudTrail 관리 이벤트로 남고,
  # 컨테이너 로그는 Docker json-file 드라이버로 로컬 보관되어 SSM 경유로
  # 조회할 수 있다. VPC 트래픽 흐름 자체의 감사 가능성만 제한된다.
  # 담당 팀: 백엔드팀(tkv00). 재검토: 2027-03-31 또는 프로덕션 전환·#132 MVP 아키텍처
