...
@ConditionalOnProperty(
	...
	name = {"enabled", "notification-fan-out.enabled"},
	havingValue = "true")
public class NotificationFanOutScheduledAdapter {
	private final NotificationFanOutWorker worker;
	...
	public NotificationFanOutScheduledAdapter(NotificationFanOutWorker worker, WorkerSchedulingProperties properties,
		WorkerInstanceIdentity identity, Clock clock, WorkerMetrics metrics) {
		this.worker = worker;
		this.settings = properties.notificationFanOut();
	...
	void runOnce() {
		try {
			...
			var result = worker.processBatch(new NotificationFanOutWorker.BatchCommand(settings.batchSize(), identity.owner(),
				at, at.plus(settings.leaseDuration()), retryPolicy));
			metrics.recordClaimed(WorkerName.NOTIFICATION_FAN_OUT, result.claimed());
			result.outcomes().forEach(outcome -> metrics.recordOutcome(WorkerName.NOTIFICATION_FAN_OUT, map(outcome), 1));
		} catch (RuntimeException failure) {
			metrics.recordOutcome(WorkerName.NOTIFICATION_FAN_OUT, Outcome.BATCH_FAILED, 1);
			throw failure;
		}
	}

	private Outcome map(NotificationFanOutWorker.Outcome outcome) {
