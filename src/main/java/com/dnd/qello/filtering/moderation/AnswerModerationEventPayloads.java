...
// #107이 발행/소비하는 outbox payload의 JSON 직렬화 지점을 한 곳으로 모은다.
// answer 콘텐츠(rawContent)는 자유 텍스트라서 다른 outbox 이벤트처럼 수동 문자열
...
final class AnswerModerationEventPayloads {
	...
	static String toJson(ObjectMapper objectMapper, Object payload) {
		try {
			...
			throw new FilteringException(FilteringErrorCode.INVALID_TEXT, "payload", "payload 직렬화에 실패했습니다");
		}
	}
	...
	static <T> T fromJson(ObjectMapper objectMapper, String json, Class<T> type) {
		try {
			...
			throw new FilteringException(FilteringErrorCode.INVALID_TEXT, "payload", "payload 역직렬화에 실패했습니다");
		}
	}
}
