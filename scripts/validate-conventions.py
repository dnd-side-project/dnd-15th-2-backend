#!/usr/bin/env python3
"""Validate branch, commit, and PR naming conventions."""
...
COMMIT_RE = re.compile(
    ...
    r"(?:\([a-z0-9-]+\))?: \[(?P<jira>[A-Z][A-Z0-9]+-\d+)\] "
    r".+ \(#(?P<issue>\d+)\)$"
)
PR_RE = re.compile(
    r"^\[(?P<jira>[A-Z][A-Z0-9]+-\d+)\] "
    r"(?:feat|fix|test|infra|docs|refactor|chore|ci|build|perf): .+$"
)
...
def match(pattern: re.Pattern[str], value: str) -> re.Match[str] | None:
...
def check_context(
    ...
    errors: list[str] = []
    branch_match = match(BRANCH_RE, branch)
    if not branch_match:
        return errors

    expected_jira = branch_match.group("jira")
    expected_issue = branch_match.group("issue")
    for message in commits:
        first_line = message.splitlines()[0]
        commit_match = match(COMMIT_RE, first_line)
        if not commit_match:
            continue
        if commit_match.group("jira") != expected_jira:
            errors.append(
                "commit Jira key does not match branch: "
                f"{commit_match.group('jira')} != {expected_jira}"
            )
    ...
    if pr_title:
        pr_match = match(PR_RE, pr_title)
        if pr_match and pr_match.group("jira") != expected_jira:
            errors.append(
                "PR Jira key does not match branch: "
                f"{pr_match.group('jira')} != {expected_jira}"
            )
...
def self_test() -> list[str]:
    errors: list[str] = []
    valid = (
        (BRANCH_RE, "chore/OPS-77-gh-9-repository-harness"),
        (BRANCH_RE, "feat/PAY-314-gh-42-dynamic-jira"),
        (COMMIT_RE, "feat(harness): [PAY-314] use branch context (#42)"),
        (PR_RE, "[PAY-314] feat: use branch context"),
    )
    invalid = (
        (BRANCH_RE, "chore/no-jira"),
    ...
    errors.extend(
        check_context(
            "feat/PAY-314-gh-42-dynamic-jira",
            ["feat(harness): [PAY-314] use branch context (#42)"],
            "[PAY-314] feat: use branch context",
            "Closes #42",
        )
    )
    mismatch_errors = check_context(
        "feat/PAY-314-gh-42-dynamic-jira",
        ["feat(harness): [BILL-9] use branch context (#7)"],
        "[BILL-9] feat: use branch context",
        "Closes #7",
    )
...
def main() -> int:
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        print("Expected branch: <type>/<JIRA-KEY>-gh-<ISSUE>-<slug>")
        print(
            "Expected commit: "
            "type(scope): [<JIRA-KEY>] short summary (#<ISSUE>)"
        )
        return 1
