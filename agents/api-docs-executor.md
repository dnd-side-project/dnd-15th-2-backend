...
완료 보고에서 변경, 산출물과 검증 결과를 제시한다. 커밋이 필요하면
`/harness-commit`으로 스펙 산출물과 그것을 바꾼 코드 변경을 같은 검토 목적으로
담은 초안을 제시하고 사람의 승인을 받는다. 이 역할은 자동으로 commit, push,
PR을 수행하지 않는다.
