...
- 요구사항에 적합한 후보만 선정해 비교한다.
- EC2/ECS 또는 RDS/자체 운영 DB는 실제 요구사항에 모두 적합한 후보일
  때만 서로 비교한다.
- 선정한 후보의 가용성, 운영 부담, 확장, 장애 복구, 비용을 비교한다.
...
- 기존의 다른 IaC를 임의로 삭제하거나 변환하지 않고 별도 마이그레이션
  Issue·계획·사람 승인을 요구한다.
