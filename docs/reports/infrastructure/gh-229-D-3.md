...
> GitHub Issue: `#229` (설계), `#233` (Terraform 구현)
> Status: Design approved — Terraform implementation in progress
> Design status: `APPROVED_FOR_BUILD` — `tkv00` 단독 승인(2026-09-16). Ownership
> 표(§10)는 `@Byuntil`·`@tkv00` 두 명의 설계 승인을 요구하나, 사용자가 이
> 세션에서 `tkv00` 단독 승인으로 확정하라고 명시적으로 지시했다. 이 승인은
> `/harness-infra-build` 진행 게이트일 뿐이며, PR 승인(`@Byuntil`, `@tkv00`)과
> `infrastructure-apply` Environment 승인은 별개로 여전히 필요하다(§10).
>
> **결정 항목 확정(2026-09-16, `tkv00`)**:
> - H-1 자동 정지 가드레일 — **포함**. EventBridge Scheduler로 매일 정해진
>   시각 EC2를 정지한다. Issue #233 범위에 반영했다.
> - H-9 비밀값 파라미터 선언 — 이 승인은 H-9 자체를 확정하지 않는다. §5 S-5의
>   권고(`value_wo` + `value_wo_version`)를 Issue #233 완료 조건에 그대로
>   반영했으며, 구현 단계에서 다시 뒤집지 않는 한 유효하다.
> - H-2(가동 시간), H-3(아키텍처), H-5(EIP), H-6(EBS 크기)는 §11 권고안을
>   그대로 채택한다.
> - H-7(8080 평문 HTTP 공개)은 이 승인에 포함해 수용한다. `SECURITY-EXCEPTION`
>   주석에 만료일과 추적 이슈를 명시하는 조건(§5 S-1)은 구현 단계에서 지킨다.
> - H-4(AWS Budgets 알림)는 이 Terraform 범위에 포함하지 않는다. 별도로 처리한다.
...
- Recommendation: **시나리오 A**를 채택한다.
  ...
  - EventBridge Scheduler로 **매일 새벽 3시(KST) 자동 정지**, 자동 시작은
    없다(H-1). 실제 가동 시간은 사람이 수동으로 시작·종료하는 시간에
    좌우되며, Terraform이 강제하는 스케줄이 아니다
  - 비용 산정은 **평일 업무 시간 160시간/월**(8시간 × 20일)이라는
    **운영 가정**을 기준으로 하며(H-2), 월 약 **9.55 USD**로 계산했다.
    이 160시간은 자동 정지 스케줄의 값이 아니라 사람이 실제로 그렇게
    쓸 것이라는 가정이다 — 가정보다 더 오래 켜두면 비용이 그만큼
    늘어난다
...
| ID | 결정 항목 | 선택지 | 권고 | 상태 |
| --- | --- | --- | --- | --- |
| **H-1** | **가동 스케줄과 자동 정지 가드레일** | (a) EventBridge Scheduler로 매일 정해진 시각 자동 정지 + 수동 시작 — 호출 횟수가 무료 구간이라 추가 비용 사실상 0, **단 Issue #229 범위에 없던 리소스(스케줄러 + 전용 IAM Role)를 추가하므로 범위 확장 승인이 필요** (b) 자동화 없이 Runbook 기반 수동 시작/정지 | **(a)** — 끄는 것을 잊으면 예산이 2.4배가 된다. 예산 상한이 얇아 가드레일 없이는 초과가 사실상 예정된 위험이다 | **확정 — (a) 채택, 범위 확장 승인됨(2026-09-16, `tkv00`)**. Issue #233 범위에 반영, `infra/modules/app-server`에 구현됨 |
| **H-2** | 월 가동 시간 상한 | 160 h(권고) / 177 h(손익분기) / 그 이상 | **160 h** (평일 8 h × 20일). 예산 여유 4.5% 확보 | **확정 — 권고안(160 h) 채택**(2026-09-16). 인스턴스 타입·비용 가정에 영향, Terraform 변수는 아님(운영 관행) |
| **H-3** | CPU 아키텍처 | x86_64 `t3.small`(권고) / Graviton `t4g.small` + 서드파티 arm64 PostGIS 이미지 | **x86_64**. 로컬 `compose.yaml`이 쓰는 `postgis/postgis`가 amd64 전용이며, 이미지를 바꾸면 dev 동등성과 공급망 검토 부담이 생긴다 | **확정 — 권고안(x86_64) 채택**(2026-09-16). `infra/modules/app-server`의 `data "aws_ami"`가 x86_64만 조회 |
| **H-4** | AWS Budgets 알림 | 이번 Terraform 범위에 포함 / 콘솔에서 사람이 수동 설정 / 별도 이슈 | **별도 이슈 또는 수동 설정**. Issue #229 범위 밖이므로 이 설계에 포함하지 않는다 | **확정 — 별도 처리**(2026-09-16). 이번 Terraform 범위(#233)에 포함하지 않음. 사람이 apply 후 콘솔에서 별도 설정 |
| **H-5** | Elastic IP 유지 | 유지(권고, 월 3.65 USD 고정) / 제거하고 자동 할당 IP 사용 | **유지**. 프론트 설정 안정성이 우선이며 Issue #229도 EIP를 명시했다 | **확정 — 권고안(유지) 채택**(2026-09-16). `infra/modules/app-server`의 `aws_eip`로 구현됨 |
| **H-6** | 데이터 EBS 크기 | 10 GiB(권고) / 그 외 | **10 GiB**(월 0.91 USD). gp3는 온라인 확장이 가능하므로 작게 시작한다 | **확정 — 권고안(10 GiB) 채택**(2026-09-16). `data_volume_size` 기본값으로 구현됨 |
| **H-7** | 8080 평문 HTTP 전체 공개 수용 | 수용 + 만료일·후속 TLS 이슈 등록 / 재설계 | **수용**. 단 `SECURITY-EXCEPTION` 주석에 만료일과 추적 이슈를 명시한다(§5 S-1) | **확정 — 수용**(2026-09-16, `tkv00`, 고위험 변경 승인). `infra/modules/app-server`의 인바운드 규칙에 만료일(2026-12-31)·추적 이슈를 명시한 `SECURITY-EXCEPTION` 주석으로 구현됨 |
| **H-8** | 설계 승인 | `APPROVED_FOR_BUILD` / 수정 요청 / 반려 | — | **확정 — `APPROVED_FOR_BUILD`**(`tkv00`, 2026-09-16, 단독 승인. Ownership 표의 2인 요구에 대한 예외는 사용자가 명시적으로 지시했다) |
| **H-9** | 비밀값 파라미터 선언 방식 | `value_wo` + `value_wo_version`(권장, §5 S-5) / `ignore_changes = [value]`(Issue #229 초안 문구, State 평문 저장을 막지 못함) / Terraform이 파라미터 값을 소유하지 않음 | **`value_wo`**. State 평문 노출을 막는 유일한 선택지 | **확정 — `value_wo` 채택**(2026-09-16). Issue #233 완료 조건에 반영, `infra/environments/dev/test-server`에 구현됨. Terraform `required_version`을 `>= 1.11.0`으로 상향 |

### 남은 절차

1. ~~`@Byuntil`, `@tkv00`가 이 보고서를 검토하고 H-1~H-9를 확정한다.~~ 완료
   (`tkv00` 단독 승인, 2026-09-16). `@Byuntil`의 별도 검토 의견은 PR
   리뷰에서 반영한다.
2. Terraform 구현이 완료되었다(#233, `gh-229-D-3-build.md`).
3. PR 생성 → `@Byuntil`·`@tkv00` 2인 승인 → `infrastructure-apply`
   Environment에서 사람이 apply.
4. apply 전 사람이 SSM SecureString 파라미터 값(`TF_VAR_db_password`,
   `TF_VAR_auth_token_secret`)을 준비한다.
5. 이 프로젝트의 AWS 계정이 `qello-new`로 바뀌어, apply 전에
   `infra/bootstrap`과 `infra/environments/dev/storage`를 그 계정에 먼저
   적용해야 한다(`gh-229-D-3-build.md` §6).
