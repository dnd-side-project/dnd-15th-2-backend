...
| 인스턴스 타입과 가동 시간 | **BLOCKED — 승인 대기** | `t3.small` 24시간 구동은 예산을 2.4배 초과한다. §6이 `t3.small` + 160시간 스케줄 구동을 **권고안**으로 제시한다. H-1·H-2가 확정되기 전에는 결정이 아니다 |
| CPU 아키텍처 | **승인 대기 (권고안 §4)** | `postgis/postgis` 매니페스트가 amd64 전용이라는 사실은 검증했다(§4). 그 근거로 x86_64를 권고하며, 확정은 H-3이다 |
...
- GitHub OIDC trust boundary: 기존 `infra/bootstrap`의 Provider와 역할을 재사용한다.
  ...
  - plan 역할에 추가: `ec2:Describe*`, `ecr:Describe*`, `ecr:GetRepositoryPolicy`,
    `ecr:ListTagsForResource`, `ssm:DescribeParameters`,
    `iam:ListInstanceProfiles*`.
  - plan 역할에는 `ssm:GetParameter*`와 파라미터 KMS Key의 `kms:Decrypt`를
    **부여하지 않는다**. `ssm:GetParameters`는 메타데이터가 아니라 값을
    반환하며, `WithDecryption=true`와 `kms:Decrypt`가 함께 있으면
    `SecureString` 평문까지 반환한다. plan은 파라미터 존재 여부와 메타데이터만
    확인하면 되므로 `ssm:DescribeParameters`로 충분하다.
...
| S-5 | 비밀값이 Terraform State에 평문으로 남을 수 있음 | **HIGH — 설계 수정** | Issue #229 완료 조건이 지정한 `ignore_changes = [value]`는 **State 저장을 막지 못한다**. `ignore_changes`는 Terraform이 외부 변경을 되돌리지 않게 할 뿐이고, refresh로 읽어온 `SecureString` 값은 State에 평문으로 기록된다(Provider 문서가 명시). AGENTS.md 4.6이 State를 민감정보로 취급하라고 요구하지만, 값을 애초에 넣지 않는 편이 낫다. **대응**: `aws_ssm_parameter`에 `value` 대신 **write-only 인자 `value_wo` + `value_wo_version`**을 쓴다. `value_wo`는 State에 저장되지 않는다. 값 자체는 사람이 콘솔이나 CLI로 넣고 Terraform에는 넘기지 않는다. `value_wo`는 Terraform 1.11.0 이상이 필요하므로 이 스택의 `required_version`을 **`>= 1.11.0, < 2.0.0`**으로 선언한다(기존 스택은 `>= 1.10.0`이며 변경하지 않는다). 값 전달 없이 파라미터를 만들 수 없다면 Terraform이 파라미터를 소유하지 않고 이름·KMS Key만 관리하는 방안을 대안으로 둔다. 변수 기본값·`terraform.tfvars`·plan·PR에 값을 쓰지 않는 원칙은 그대로 유지한다 |
...
- Drift detection: 정기 자동 검사는 두지 않는다. PR마다 실행하는 plan이
  drift 탐지 역할을 겸한다.
- 버전 고정: AWS Provider 버전과 `.terraform.lock.hcl`을 기존 스택과 동일한
  방식으로 커밋한다(현재 저장소 기준 AWS Provider 6.57.1). Terraform
  `required_version`은 이 스택만 **`>= 1.11.0, < 2.0.0`**으로 올린다.
  `aws_ssm_parameter`의 write-only 인자 `value_wo`가 1.11.0 이상을 요구하기
  때문이다(§5 S-5). 기존 스택의 `>= 1.10.0` 제약은 건드리지 않는다.
...
| **H-9** | 비밀값 파라미터 선언 방식 | `value_wo` + `value_wo_version`(권장) / Issue #229가 적은 `ignore_changes = [value]` 유지 / Terraform이 파라미터 값을 소유하지 않음 | **`value_wo`**. `ignore_changes`는 State 평문 저장을 막지 못한다(§5 S-5). Issue #229 완료 조건 문구와 달라지므로 Issue 본문도 함께 정정해야 한다 | **미결정 — Issue 완료 조건 변경 필요** |
...
| 2026-09-16 | PR #232 리뷰 반영: §1 Intake의 `DECIDED` 표기를 승인 대기로 되돌리고, plan 역할에서 `ssm:GetParameters`를 제거하고, S-5를 `value_wo` 기반으로 다시 쓰고, H-9를 추가했다 | 승인 전 항목을 결정으로 표기했고, `ssm:GetParameters`를 메타데이터 전용으로 잘못 기술했으며, `ignore_changes = [value]`가 State 평문 저장을 막는다고 잘못 전제했다 |
