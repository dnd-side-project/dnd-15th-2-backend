...
- **#251**: 두 명 모두의 승인을 요구하면 PR 작성자 본인은 GitHub
  제약상 자신의 PR을 승인할 수 없어 그 조건을 구조적으로 영원히
  충족할 수 없었다. `@Byuntil` 또는 `tkv00` 중 한 명의 승인으로
  완화했다(`AGENTS.md` 4.8절도 함께 갱신).
- **#249 무효화**: #247/#249는 열린 채로 유지해 apply 승인 근거로
  쓰려는 의도였으나 실수로 머지되어 더 이상 "열린 PR" 조건을
  만족하지 못하게 됐다. **#253**이 그 대체 근거다 — #253은 apply
  완료 전까지 머지하지 않는다.
- **#255**: 첫 실제 apply 실행 로그에 `media_bucket_kms_key_arn`(계정
  ID가 포함된 전체 ARN)이 마스킹 없이 노출됐다. GitHub Actions는
  secret만 로그를 마스킹하고 variable은 마스킹하지 않는다 —
  Terraform의 `sensitive` 여부와 무관하다. secret으로 옮겨 해결했다.
- **#253/#254 무효화**: #253/#254도 #249와 같은 이유로 실수로 머지돼
  더 이상 열린 PR 조건을 만족하지 못한다. **#257**이 그 대체 근거다
  — #257 역시 apply 완료 전까지 머지하지 않는다.
