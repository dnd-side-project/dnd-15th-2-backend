...
1. `FlywayMigrationContractTest` — 마이그레이션 파일 목록이 고정 목록이라 V22 추가로
   실패했다. 목록에 등록해 해소했다.
2. `FlywayMigrationIntegrationTest` — 적용 개수(20 → 21)와 FK 총계(54 → 55)가 고정값이라
   실패했다. `user_account`가 매니페스트 대상 테이블이라 V22의 FK가 총계에 반영된다.
...
- V22는 컬럼 추가와 FK 추가만 하고 데이터를 변환하지 않는다. 기존 행은 모두 `NULL`이라
  기본 이미지 상태가 된다.
