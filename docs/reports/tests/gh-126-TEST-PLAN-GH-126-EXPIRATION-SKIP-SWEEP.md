...
- 두 worker 모두 실패한 행을 batch 요약 로그 한 줄로만 남기고 별도 알림·재시도
  큐로 보내지 않는다. 로그 볼륨을 제한하기 위해 stack trace는 batch당 첫 실패
  하나만 첨부하므로, 한 batch 안에서 원인이 다른 실패가 섞이면 두 번째 이후
  원인은 로그에 남지 않고 `failed` 카운터로만 보인다. 실패가 반복되는 특정 행이
  있다면 다음 sweep 실행에서 다시 후보로 잡혀 재시도되지만, 그 사이 관측은
  요약 로그와 카운터에 의존한다.
